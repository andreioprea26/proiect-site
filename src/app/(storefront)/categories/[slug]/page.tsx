...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-12 sm:px-8 sm:py-16">
      <Link className="text-sm font-semibold text-brand hover:underline" href="/categories">← Toate categoriile</Link>
      <header className="mb-10 mt-6 max-w-3xl">
        <p className="text-sm font-semibold uppercase tracking-[0.18em] text-brand">Categorie</p>
        <h1 className="mt-3 text-4xl font-semibold tracking-tight sm:text-5xl">{result.taxonomy.name}</h1>
        {result.taxonomy.description ? <p className="mt-5 text-lg leading-8 text-brand-muted">{result.taxonomy.description}</p> : null}
      </header>
      <ProductGrid
