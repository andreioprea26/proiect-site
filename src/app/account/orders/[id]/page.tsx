...
export default async function CustomerOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <Link className="text-sm font-semibold text-brand hover:underline" href="/account/orders">← Comenzile mele</Link>
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2"><section className="rounded-xl border border-brand-border/25 bg-brand-surface p-6"><h2 className="text-xl font-semibold">Livrare</h2><address className="mt-4 not-italic leading-7 text-brand-muted">{addressLines(order.shippingAddress).map((line) => <span className="block" key={line}>{line}</span>)}</address>{order.shipment ? <div className="mt-5 border-t border-stone-100 pt-4 text-sm"><p><strong>Curier:</strong> {order.shipment.carrier ?? "—"}</p><p className="mt-1"><strong>AWB:</strong> {order.shipment.trackingNumber ?? "—"}</p>{order.shipment.trackingUrl ? <a className="mt-2 inline-block font-semibold text-brand hover:underline" href={order.shipment.trackingUrl} rel="noreferrer" target="_blank">Urmărește expedierea ↗</a> : null}</div> : <p className="mt-4 text-sm text-stone-500">Tracking-ul va apărea după pregătirea expedierii.</p>}</section>
