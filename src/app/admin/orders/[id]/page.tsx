...
import { CodCollectionAction } from "@/app/admin/_components/cod-collection-action";
import { NotificationHistory } from "@/app/admin/_components/notification-history";
...
import { allowedOrderTransitions, canCancelOrder, canCollectCod, canConfigureShipment, canMarkOrderShipped, canRefundStripe, ORDER_STATUS_LABELS, PAYMENT_METHOD_LABELS, PAYMENT_STATUS_LABELS, orderStatusBadgeClass } from "@/lib/admin/order-model";
...
export default async function AdminOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const collectCodAvailable = canCollectCod({ status: order.status, paymentMethod: order.paymentMethod, paymentStatus: order.paymentStatus, collectionStatus: order.codCollection?.status ?? null });
  const retryRequestIds = Object.fromEntries(order.notifications.map((notification) => [notification.id, randomUUID()]));
  ...
  return (
    <div>
      ...
      <section className="mt-6 rounded-2xl border border-stone-800 bg-stone-900 p-6"><h2 className="text-xl font-semibold">Notificări</h2><p className="mt-2 text-sm text-stone-400">Destinatarul provine exclusiv din snapshot-ul comenzii. În Development/Preview livrarea este redirecționată către adresa de test configurată.</p><NotificationHistory notifications={order.notifications} orderId={order.id} retryRequestIds={retryRequestIds} /></section>
    </div>
  );
}
